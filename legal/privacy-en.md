...
The App has no user accounts and no login flow — there is no account you create, no login process, and no email address or password you ever provide.
...
This data is **not linked to your identity**. The Firebase Installation ID and the Install ID are separate per-install random identifiers, and neither is connected to your Google account, advertising ID, or any other identifier. Both are reset when you uninstall the App or clear its data.

The App makes anonymous network requests to **Cloud Firestore** to download exam questions, categories, and explanations; these requests carry no information that identifies you personally. When you vote on an explanation or send a problem report, the App additionally writes that vote or report to Cloud Firestore as described above — what we store includes your Install ID and the vote or report data, but no information that identifies you personally. All Cloud Firestore traffic is protected by Firebase App Check (Play Integrity on Android, DeviceCheck on iOS).
