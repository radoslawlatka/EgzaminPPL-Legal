...
The App has no user accounts and no login flow — there is no account you create, no login process, and no email address or password you ever provide. Instead, the App creates an invisible, anonymous technical identifier in the background, described in section 3.

Beyond the anonymous technical and diagnostic data described in section 3, the only information that ever leaves your device is feedback you actively choose to send: a thumbs-up or thumbs-down on an explanation, a problem report about a question, or an email you send us from Settings. Votes and problem reports are anonymous — never linked to your identity. An email, by its nature, comes from your own address, and you decide exactly what it contains. All of this is optional and is described in section 3.

## 3. What we do collect

We collect a minimal amount of **anonymous technical and diagnostic data** to keep the App stable and — if you choose to use these features — your **explanation votes** and **problem reports**.
...
| Anonymous authentication identifier (a randomly generated identifier per app install, created silently in the background and invisible to you) | A stable anonymous identity that future App features can rely on — for example, attaching an optional subscription purchase — without identifying you | Firebase Authentication |
...
This data is **not linked to your identity**. The Firebase Installation ID, the Install ID, and the anonymous authentication identifier are separate per-install random identifiers, and none of them is connected to your Google account, advertising ID, or any other identifier. The Firebase Installation ID and the Install ID are reset when you uninstall the App or clear its data. The anonymous authentication identifier is reset the same way on Android; on iOS it is held in the system keychain, so it may survive a reinstall — until the device's keychain data is removed.

The App makes anonymous network requests to **Cloud Firestore** to download exam questions, categories, and explanations; these requests may carry an anonymous authentication token — it identifies the installation's anonymous identity, not you — and carry no information that identifies you personally. When you vote on an explanation or send a problem report, the App additionally writes that vote or report to Cloud Firestore as described above — what we store includes your Install ID and the vote or report data, but no information that identifies you personally. All Cloud Firestore traffic is protected by Firebase App Check (Play Integrity on Android, App Attest on iOS).
...
- **Anonymous authentication identifier** — our interest in operating and developing the App, including preparing the groundwork for future features such as attaching an optional subscription purchase to a given installation.
...
We have weighed this interest against your privacy. The processing uses only the data it needs, relies on a random per-install identifier rather than anything that identifies you, keeps your votes and reports private, and lets the content you write yourself leave the device only when you choose to vote, send a report, or write to us. We therefore consider that it does not override your interests, rights, or freedoms.

## 5. Who processes the data

Technical and diagnostic data, explanation votes, and problem reports are processed on our behalf by:

- **Google LLC / Google Ireland Limited** - provider of Firebase Crashlytics, Firebase Installations, Firebase Authentication, Cloud Firestore, and Firebase App Check. Google's privacy practices are described at [policies.google.com/privacy](https://policies.google.com/privacy) and [firebase.google.com/support/privacy](https://firebase.google.com/support/privacy).
...
- The anonymous authentication identifier is reset on Android whenever you uninstall the App or clear its storage. On iOS it is held in the system keychain, so it may survive a reinstall — until the device's keychain data is removed.
...
Uninstalling the App also resets your local Install ID. Any votes or problem reports you have already sent stay stored, but they are linked only to that random local identifier, which is not kept anywhere else — once it is gone, those votes and problem reports can no longer be traced back to you or your device through that identifier. This does not apply to the anonymous authentication identifier described in sections 3 and 6, which is separate from the Install ID: it is reset the same way on Android, but on iOS it is held in the system keychain, so it may survive a reinstall — until the device's keychain data is removed.
